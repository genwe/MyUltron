...
- (instancetype)init {
    return [super initWithFeatureName:@"消息推送"];
}
...
- (void)updateStatusForConnection {
    dispatch_async(dispatch_get_main_queue(), ^{
        if (self.client.isConnected) {
            self.statusLabel.stringValue = @"已连接，编辑 JSON 后点击发送";
...
- (void)setupUI {
    ...
    inputLabel.stringValue = @"推送 Payload (JSON):";
    ...
    // ---- 发送按钮 ----
    _sendButton = [NSButton buttonWithTitle:@"发送"
                                     target:self
...
- (void)sendPush:(NSButton *)sender {
    ...
    if (jsonText.length == 0) {
        self.statusLabel.stringValue = @"请输入 JSON payload";
        return;
    }
    ...
    if (error || ![payload isKindOfClass:[NSDictionary class]]) {
        self.statusLabel.stringValue = [NSString stringWithFormat:@"JSON 格式错误: %@", error.localizedDescription];
        return;
    }
...
- (void)didReceiveMessage:(NSDictionary *)dict {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (success) {
            self.statusLabel.stringValue = @"推送已送达 AppDelegate";
        } else {
            NSString *err = content[@"error"] ?: @"未知错误";
            self.statusLabel.stringValue = [NSString stringWithFormat:@"发送失败: %@", err];
        }
    });
}
